...
export default function MenuItemCard({ item, lang = 'am', t }) {
  ...
  const fallbackImg =
    'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="300" height="300" viewBox="0 0 24 24" fill="none" stroke="%23f97316" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="18" height="18" x="3" y="3" rx="2" ry="2"/><circle cx="9" cy="9" r="2"/><path d="m21 15-3.086-3.086a2 2 0 0 0-2.828 0L6 21"/></svg>';
  ...
  if (!item.hasVariants || !item.variants || item.variants.length === 0) {
    ...
    return (
      <div
        className={`relative bg-zinc-900 border border-zinc-800 rounded-2xl md:rounded-[2.5rem] shadow-xl overflow-hidden hover:shadow-2xl transition-all duration-500 flex flex-col h-full w-full ${
          ...
          <div>
            ...
            <div className="flex items-center gap-1 md:gap-2 text-zinc-300 text-[9px] sm:text-xs md:text-sm mb-2 md:mb-8 font-medium">
              ...
              <span className="break-words">
                {t?.freshAndSelected || 'ትኩስ እና የተመረጠ'}
              </span>
            </div>
          </div>

          <div className="flex gap-1 md:gap-4 mt-auto items-center">
            {countInCart > 0 && isAvailable && (
              <button
                type="button"
                onClick={(e) => {
                  e.stopPropagation();
            ...
            <button
              ...
              onClick={(e) => {
                e.stopPropagation();
                if (isAvailable) addToCart(mainName, item.price);
              }}
              className={`flex-grow py-1.5 md:py-4 rounded-lg md:rounded-2xl font-black text-[10px] sm:text-xs md:text-base flex items-center justify-center gap-1 md:gap-2 transition-all shadow-md ${
                isAvailable
                  ? 'bg-orange-500 hover:bg-orange-600 text-white cursor-pointer shadow-orange-950/30'
  ...
  return (
    <div
      ...
      className={`min-h-[230px] sm:h-[280px] md:h-[490px] w-full [perspective:1000px] group select-none ${
      ...
      >
        ...
        >
          ...
          <div className="relative h-24 sm:h-36 md:h-64 rounded-xl md:rounded-3xl overflow-hidden mb-1 md:mb-4">
            ...
            />
            {isAvailable && (
              <div className="absolute top-1 right-1 md:top-4 md:right-4 bg-black/80 border border-zinc-700 text-orange-400 text-[8px] sm:text-[10px] md:text-xs px-2 py-0.5 md:px-3 md:py-1.5 rounded-full font-bold flex items-center gap-1">
                <RotateCw size={10} />
              </div>
            )}
          </div>
        ...
        <div className="absolute inset-0 w-full h-full bg-zinc-950 text-white rounded-2xl md:rounded-[2.5rem] p-2 sm:p-3 md:p-6 [backface-visibility:hidden] [transform:rotateY(180deg)] flex flex-col justify-between border border-orange-500/40 shadow-2xl">
          <div>
            <div className="text-center mb-1 md:mb-4 pb-1 md:pb-3 border-b border-zinc-800">
              <h3 className="text-xs sm:text-base md:text-2xl font-black text-orange-400 break-words leading-tight">
                {mainName}
              </h3>
            </div>
                ...
                return (
                  ...
                  >
                    <div className="overflow-hidden mr-1 flex-1">
                      ...
                      <p className="text-orange-400 font-black text-[9px] sm:text-xs md:text-sm">
                        {v.price} ETB
                      </p>
                    </div>
                    ...
                    <div className="flex items-center gap-0.5 md:gap-2 shrink-0">
                      ...
                      <span className="font-black text-[9px] md:text-xs w-3 md:w-4 text-center text-white">
                        {countInCart}
                      </span>
