...
export default function MyOrder({ isOpen, onClose }) {
  ...
  const loadMyOrders = () => {
    try {
      ...
      console.error("Orders load madalu agilla:", e);
    }
  };

  const handleClearHistory = () => {
    if (window.confirm("በእርግጥ ሁሉንም የትዕዛዝ ታሪክ ማጥፋት ይፈልጋሉ?")) {
          ...
          {orders.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {orders.map((order, idx) => {
                ...
                const paymentMethodStr = getPaymentText(order);
